...
class Local_Planner():
    def __init__(self):
        ...
        # self.__sub_goal_state = rospy.Subscriber('/demo_node/global_path', Float32MultiArray, self._global_path_callback, queue_size=10)
        self.__sub_goal_state = rospy.Subscriber('/surf_predict_pub', Float32MultiArray, self._global_path_callback2, queue_size=10)

        # self.__ob = rospy.Subscriber('/local_obs_pub/obs_vis', Float32MultiArray, self.__ob_cb, queue_size=10)
        # self.__sub_map = rospy.Subscriber('/map',OccupancyGrid,self.__curr_map_cb,queue_size=10)
        ...
        # sub = rospy.Subscriber("/urg_scan", LaserScan, self._scan_callback, queue_size=10)
        
        
        self.control_cmd = Twist()
        # self.__pub_force_cmd = rospy.Publisher('/local_planner/force_cmd', Float64, queue_size=10)
    ...
    def draw_ob(self):
        ...
        for i in range(len(self.ob)):
            ...
            num = num +1
            # print("position : ",  se`lf.ob[i][0],  self.ob[i][1])
        # print(self.obstacle_markerarray.markers)
    ...
    def __obs_cb(self, data):
        ...
        if(len(data.data)!=0):
            ...
            self.draw_ob()
            # print(size,' ',len(self.ob))
        # obs_num = len(data.data)/3
        # s = set()
        # for i in range(obs_num):
        #     s.append((data.data[3*i],data.data[3*i+1])
        # print(s)
    ...
    def choose_goal_state(self):
        ...
        # print(self.desired_global_path[1])
        # print(num+15)
        # num_path = min(self.desired_global_path[1],num+14)
        scale = 1
        num_list = []
        for i in range(self.N):  
